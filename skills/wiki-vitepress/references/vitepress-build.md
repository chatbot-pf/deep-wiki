# VitePress Build — Full Reference

Complete implementation reference for packaging generated wiki Markdown into a VitePress static site with a Daytona-inspired dark theme, dark-mode Mermaid diagrams, and click-to-zoom. The parent `SKILL.md` is the concise overview; this file holds the full config, theme, CSS, and scripts. Load it when actually scaffolding or building the site.

## Prerequisites

The wiki markdown files should already exist (from `/deep-wiki:generate` or manual creation). The build scaffolds the VitePress project around them.
...
export default {
  ...
  setup() {
    ...
    const openDiagramModal = (svg: SVGSVGElement) => {
      ...
      // Pan handlers are registered on document and removed in closeOverlay to avoid leaks
...
- **Remove every document-level listener in `closeOverlay`** — the `keydown`, `mousemove`, and `mouseup` handlers are registered on `document`, so the close path must detach all three or they leak across opens
