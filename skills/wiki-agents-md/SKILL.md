...
- **Repository root** (`/`)
- **Wiki folder** (`wiki/`) — if generated by deep-wiki (has a `package.json` for the site generator: VitePress, Nextra, or Astro Starlight)
...
Use this template (adapt to the actual project). Fill the **site-generator-specific** rows from the chosen adapter's manifest (`skills/wiki-site-core/references/adapters/<tool>.md`) — do not assume VitePress. The example below shows VitePress (the default); for Nextra use `next build` / `next.config.mjs`, for Starlight use `astro build` / `astro.config.mjs` (see the "Generator-specific values" note after the template):

```markdown
# Wiki — Agent Instructions

## Overview
Generated documentation site (<GENERATOR> — VitePress, Nextra, or Astro Starlight). Contains architecture docs, onboarding guides, and API references with source-linked citations and dark-mode Mermaid diagrams.

## Build & Run
- Install: `npm install`   <!-- manifest install_cmd -->
- Dev server: `npm run dev`
- Build: `npm run build`    <!-- manifest build_cmd, e.g. `next build` / `astro build` -->
...
- `<GENERATOR CONFIG>` — Site config with sidebar and Mermaid setup (e.g. `.vitepress/config.mts`, `next.config.mjs` + `theme.config`, or `astro.config.mjs`)
- `<GENERATOR THEME>` — Dark theme / design tokens (e.g. `.vitepress/theme/`)
...
- ⚠️ **Ask first:** Change theme CSS/design tokens, modify the generator config, restructure sections
...
**Generator-specific values** — replace `<GENERATOR>`, the Build & Run commands, and the config/theme paths from the chosen adapter's manifest:

| Generator | `build_cmd` | Config file | Theme / tokens |
|-----------|-------------|-------------|----------------|
| VitePress | `npm run build` | `.vitepress/config.mts` | `.vitepress/theme/` |
| Nextra | `next build` | `next.config.mjs` + theme config | `mdx-components.js` / CSS tokens |
| Starlight | `astro build` | `astro.config.mjs` | Starlight `customCss` / theme plugin |
